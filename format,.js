...
function getAllPhotos() {
  ...
  getPhotos916().forEach(function(src, i) {
    all.push({
      src:  src,                           // Thumbnail für Galerie
      full: fullArr[i] || src,             // Vollbild für Postkarte
      lbl:  "Foto " + (i+1),
      custom: true, idx: i
    });
  });
  ...
  photos.forEach(function(item, idx) {
    ...
    div.onclick = (function(s, f, d) {
      return function() {
        ...
        window.pcSelectedPhoto = f || s; // Vollbild für Postkarte
        var cv = document.getElementById("pcanv");
        if (cv) { cv.width = 540; cv.height = 960; }
        if (typeof renderPC === "function") renderPC(true);
      };
    })(item.src, item.full, div);
    row.appendChild(div);
